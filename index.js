        ...
        allAnnouncements.forEach((item, index) => {
            ...
            const imageDisplay = item.image_url 
                ? `<div style="margin: 20px 0; text-align: center;">
                    <img src="http://localhost:3000/uploads/${item.image_url}" 
...
if (annForm) {
    annForm.addEventListener('submit', async (e) => {
        ...
        const url = id ? `http://localhost:3000/announcements/${id}` : 'http://localhost:3000/announcements';
...
window.deleteAnn = async function(id) {
    if (confirm("Delete this announcement? This will send a notification to staff.")) {
        try {
            const response = await fetch(`http://localhost:3000/announcements/${id}`, { method: 'DELETE' });
            if (response.ok) {
                alert("🗑️ Deleted!");
                loadAnnouncements();
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchEmailBtn) {
        searchEmailBtn.addEventListener('click', async () => {
            ...
            if (!email) return;
            try {
                const res = await fetch(`http://localhost:3000/emails/check/${encodeURIComponent(email)}`);
